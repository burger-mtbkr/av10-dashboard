...
describe('SubwooferCard', () => {
  it('should render the title', () => {
    renderWithProviders(
      <SubwooferCard subwoofers={[]} lfeLevel="0 dB" />,
    );
    expect(screen.getByText('Subwoofer Settings')).toBeInTheDocument();
  });

  it('should display LFE level', () => {
    renderWithProviders(
      <SubwooferCard subwoofers={[]} lfeLevel="-5 dB" />,
    );
    expect(screen.getByText('LFE: -5 dB')).toBeInTheDocument();
  });
