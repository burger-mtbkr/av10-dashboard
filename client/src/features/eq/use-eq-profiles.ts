...
import { clampEqGainDb } from "./gain-range";
import type {
  IEqBand,
  IEqProfile,
  IEqProfilesResponse,
...
export const useEqProfiles = (
  ...
  const applyProfile = useCallback(async () => {
    ...
    setStatusMessage("");
    try {
      const applied = await applyEqProfileRequest(activePreset, selectedProfile.id);
      setDraftBands(cloneBands(applied.bands?.length ? applied.bands : selectedProfile.bands));
