...
import { CHANNEL_MAP, OPINFASP_CHANNEL_ORDER, PLACEHOLDER_VALUE, SOURCE_MAP, SMART_SELECT_DEFAULTS, SMART_SELECT_SLOTS, TELNET_EVENT_MAP, parseVolume, volumeToCommand } from '../core/constants.js';
import { fetchHttpStatus } from '../api/status/fetch-http-status.js';
import type { IAVRStatus, IInputSource, ISmartSelectPreset, ISpeakerStatus, ITelnetEvent } from '../types/core.js';
import type { IEqBand, IEqProfile, SpeakerPreset } from '../types/eq.js';
import { clampGainDb } from '../features/eq/validators.js';
import {
  ...
  async applyEqProfile(
    ...
    try {
      ...
      for (const band of profile.bands) {
        this.sendCommand(formatGraphicEqBandCommand(band.frequencyHz, band.gainDb));
        await new Promise((r) => setTimeout(r, 90));
      }
      const bands = profile.bands.map((b) => ({
          frequencyHz: b.frequencyHz,
          gainDb: clampGainDb(b.gainDb),
      }));
      const verifiedBands = 0;
      this.status.graphicEq = {
        bands,
        updatedAt: new Date().toISOString(),
      ...
      this.status.lastUpdate = new Date().toISOString();
      this.emit('statusChanged', this.status);
      return { sent: profile.bands.length, profileId: profile.id, bands, verifiedBands };
    } finally {
